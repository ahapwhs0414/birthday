...
export default function BirthdayJourney() {
  ...
  const restart = () => { setError(""); setShareNotice(""); setDraft(initialDraft()); window.scrollTo(0, 0); };
  ...
  return <main className="shell">
    ...
    <section className="card" aria-live="polite">
      ...
      {draft.step === "completed" && <><Eyebrow>{draft.testMode ? "테스트 완료" : "축하 배달 완료"}</Eyebrow><h1>예진이의 생일을 축하해주셔서 감사합니다!</h1><div className="completion-photo" aria-hidden="true">🎉</div>{draft.testMode && <p className="notice">테스트 참여는 저장되지 않아 예진이의 보관함에 표시되지 않아요.</p>}<div className="result-summary"><div><small>시험</small><strong>{draft.examScore}점</strong></div><div><small>실습</small><strong>{practiceScore}/3</strong></div></div><blockquote>{draft.message}</blockquote><button className="primary" onClick={share}>친구에게 참여 링크 보내기</button>{draft.testMode && <button className="secondary" onClick={restart}>처음부터 다시 하기</button>}{shareNotice && <p className="notice">{shareNotice}</p>}</>}
    </section>
