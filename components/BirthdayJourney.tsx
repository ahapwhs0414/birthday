...
export default function BirthdayJourney() {
  ...
  const [seconds, setSeconds] = useState(birthdayContent.exam.timeLimit);
  ...
  return <main className="shell">
    ...
    <section className="card" aria-live="polite">
      ...
      {draft.step === "practice" && practiceRound && <><Eyebrow>준비물 {draft.answers.length + 1} / 3</Eyebrow><div className="avatar"><span>👩🏻‍⚕️</span>{draft.answers.map((answer) => <small key={answer.roundId}>{birthdayContent.practice.find(r => r.id === answer.roundId)?.options.find(o => o.id === answer.choiceId)?.emoji}</small>)}</div><h1>{practiceRound.prompt}</h1><div className="practice-grid">{practiceRound.options.map((option) => <button key={option.id} onClick={() => choosePractice(option.id)}><b>{option.emoji}</b><span>{option.label}</span></button>)}</div></>}
...
function Story({ index, previous, next }: { index: number; previous: () => void; next: () => void }) {
  ...
  return <div className="story" onTouchStart={(event) => { touchStart.current = event.touches[0].clientX; }} onTouchEnd={endSwipe}><Eyebrow>생일 이야기</Eyebrow><div className="story-art"><span>{item.emoji}</span><i>{index === 0 ? "22" : ""}</i></div><h1>{item.title}</h1><p>{item.detail}</p><div className="dots">{birthdayContent.story.map((_, i) => <i key={i} className={i === index ? "active" : ""} />)}</div><div className="actions">{index > 0 && <button className="secondary" onClick={previous}>이전</button>}<button className="primary" onClick={next}>{index === 3 ? "예진이 도와주기" : "다음 이야기"}</button></div></div>;
}
